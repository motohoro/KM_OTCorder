//document.body.style.border = "5px solid blue";
//var script = document.createElement('script');
//script.src = 'https://code.jquery.com/jquery-2.1.1.min.js';
//document.body.appendChild(script);

jQuery(document).ready(function () {
document.body.style.border = "5px solid red";
/*
//$("body").prepend('<script type="text/javascript" src="https://rawgit.com/schmich/instascan-builds/master/instascan.min.js"></script>');
//document.getElementById("scanbtn").addEventListener("click",function(e){console.log("SCANBTNCLICK")});
*/
if($("#preview").length){
return;
}
$("body").prepend('<div><video id="preview" width="300" height="150" style="text-align:center;display:none"></video></div><div><button id="scanbtn">SCAN</button></div><div id="scannamelist"></div>');
var script3 = document.createElement('script');
script3.type="text/javascript";
script3.innerHTML = `function aaa() {
$(window).on('beforeunload', function() {
	cameraoff();
	return;
});

$("#scanbtn").on('click',function(){
		$("#preview").toggle();
		if($("#preview").css('display')=="none"){
			$("#scanbtn").text("SCAN");
			cameraoff();
		}else{
			$("#scanbtn").text("SCAN OFF");
			camerainit();
		}
	});
	camerainit = function(){
      Instascan.Camera.getCameras().then(function (cameras) {
        if (cameras.length > 0) {
          scanner.start(cameras[0]);
        } else {
          console.error('No cameras found.');
        }
      }).catch(function (e) {
        console.error(e);
      });
    
    }/*camerainit*/
	cameraoff = function(){
		scanner.stop();
	}/*cameraoff*/
console.log("Instascan before");
let scanner;
try{
scanner = new Instascan.Scanner({ video: document.getElementById('preview'),backgroundScan:true,scanPeriod: 2 });
}catch(e){
alert("reload");
return;
}
console.log("Instascan after");
      scanner.addListener('scan', function (content) {
      console.log(content);
      	qrdatajson = JSON.parse(content);
      	qrdataparam=qrdatajson['param'].join("&");
/*      	console.log(qrdatajson['name']);*/
/*      	console.log(qrdataparam);*/
      // http://qiita.com/isseium/items/12b215b6eab26acd2afe
      new Audio("data:audio/wav;base64,").play();
		/*
		var insel= document.createElement("div");
		//https://stackoverflow.com/questions/22587308/convert-iso-8859-1-to-utf-8
//		insel.innerHTML=decodeURIComponent(escape(qrdatajson['name']));
		insel.innerHTML=decodeURIComponent(escape(qrdatajson['name']));
		$("#scannamelist").insertBefore(insel,parent.firstChild);
		*/
		$("#scannamelist").append("<div>"+decodeURIComponent(escape(qrdatajson['name']))+"</div>")
/*//		$("div",{text:decodeURIComponent(escape(qrdatajson['name']))}).appendTo('#scannamelist');
// var win = window.open("","orderw");
// var win = window.open("https://kansaimedico.co.jp/medico_syanai/cgi/spfne/shop.cgi?" + qrdataparam,"orderw");
*/
		$.get("https://kansaimedico.co.jp/medico_syanai/cgi/spfne/shop.cgi?" + qrdataparam);
      });

};
`;
document.head.appendChild(script3);
/*
// http://mayer.jp.net/?p=4396
var script0 = document.createElement('script');
script0.type="text/javascript";
script0.src = 'https://cdn.rawgit.com/polygonplanet/encoding.js/31732f52/encoding.min.js';
document.head.appendChild(script0);
*/
var script = document.createElement('script');
script.type="text/javascript";
script.setAttribute('onload',`var script2 = document.createElement('script');
script2.id="jqueryscript";
script2.type="text/javascript";
script2.setAttribute('onload',"aaa();");
script2.src = 'https://code.jquery.com/jquery-3.2.1.min.js';
document.head.appendChild(script2);
`);
//script.src = 'https://rawgit.com/schmich/instascan-builds/master/instascan.min.js';
script.src = 'https://cdn.rawgit.com/schmich/instascan-builds/36ad50f0/instascan.min.js';
document.head.appendChild(script);
/*
var script2 = document.createElement('script');
script2.id="jqueryscript";
script2.type="text/javascript";
script2.setAttribute('onload',"aaa();");
script2.src = 'https://code.jquery.com/jquery-3.2.1.min.js';
document.head.appendChild(script2);
*/

});
